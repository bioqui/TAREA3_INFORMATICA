...
##                                                                                ##
## Autor: Francisco J. Romero-Campero Adaptation: Juan Fco. Alba & Claudia Muñoz  ##
## Email: juanfav98@gmail.com                                                     ##
## Fecha: Febrero 2018                                                            ##
####################################################################################
...
for(i in 1:length(tfs))
{
  ...
  current.tf.data <- read.table(file=current.tf.file,as.is=TRUE)
  current.tf.targets <- current.tf.data[,1]
  adj.matrix[unlist(current.tf),current.tf.targets] <- 1
}
...
tfs.network <- induced.subgraph(gene.transcriptional.network,unlist(tfs))
...
network.degrees <- degree(random.graph)
hist(network.degrees,col="blue",xlab="Node degree", ylab="Probability",main="Degree distribution")
...
## Hemos probado todos y ninguno es significativo los que mas se acercan son los de autorregulación.
